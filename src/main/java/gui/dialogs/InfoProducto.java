...
public class InfoProducto extends javax.swing.JDialog {
    ...
    public InfoProducto(String codigo) {
        initComponents();
        this.codigo = codigo;
        this.idProducto = Consultar.ConsultarIdArticulo(codigo);
        this.infoProducto = Consultar.ConsultarInfoProducto(codigo);
    ...
    private void initComponents() {
        ...
        jLabel_MasCantidad.setText("Agregar la cantidad de:");
        ...
        getContentPane().add(jButton_Actualizar, new org.netbeans.lib.awtextra.AbsoluteConstraints(290, 170, 110, 100));
    ...
    private void jButton_ActualizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_ActualizarActionPerformed
        String cantidad = txt_CantidadActual.getText().trim();
        String masCantidad = txt_MasCantidad.getText().trim();
        codigo = txt_Codigo.getText().trim();
        String nombre = txt_Nombre.getText().trim();
        String precio = txt_Precio.getText().trim();

        if ((txt_MasCantidad.getText().trim().equals(""))) {
            masCantidad = "0";
        }

        if ((txt_Nombre.getText().trim()).equals("") || (txt_Codigo.getText().trim()).equals("")
                || (txt_Precio.getText().trim()).equals("") || (txt_CantidadActual.getText().trim()).equals("")) {

            JOptionPane.showMessageDialog(null, "Debes llenar todos los campos");
        } else {
            if (Consultar.ConsultarCodigoArticulo(idProducto, codigo)) {
                JOptionPane.showMessageDialog(null, "El código no esta disponible, intenta con otro");
            } else {
                String nCantidad = String.valueOf(Double.parseDouble(cantidad) + Double.parseDouble(masCantidad));

                Actualizar.ActualizarInfoProducto(idProducto, new String[]{codigo, nombre, nCantidad, precio, usuario});
                this.dispose();
            }
        }
    ...
    private void initTextFields() {
        txt_Nombre.setText(infoProducto[0]);
        txt_Codigo.setText(codigo);
        txt_CantidadActual.setText(infoProducto[1]);
        txt_Precio.setText(infoProducto[2]);
        txt_UltimaActualizacion.setText(infoProducto[3]);
    }
