...
//            System.err.println("Error en la conexión: " + ex.getMessage());
    ...
    public static String ObtenerVersion() {
        try {
            ...
            con.connect();
            return ObtenerContenidoURL(urlVerificar);
        } catch (MalformedURLException ex) {
//            System.err.println("Error en url: " + ex.getMessage());
        } catch (IOException ex) {
//            System.err.println("Error en la conexión: " + ex.getMessage());
     ...
     */
    public static String ObtenerContenidoURL(URL urlVerificar) {
        try {
    ...
    public static void EnlazarDescarga() {
        try {
            Desktop.getDesktop().browse(new URI("https://github.com/arambula357/Sankarasel/releases/download/v1.1.0/setupSankarasel.exe"));
